# PROMPT — ABYSS INVADERS: YELLOW SUBMARINE EDITION

> Envie este documento integralmente ao modelo. Ele contém toda a especificação técnica,
> estética e de gameplay necessária para recriar o jogo em um único arquivo HTML autocontido.
...
```
creature1 → assets/abyssal\_creatures\_sprites/Creature\_1.png
creature2 → assets/abyssal\_creatures\_sprites/Creature\_2.png
creature3 → assets/abyssal\_creatures\_sprites/Creature\_3.png
creature4 → assets/abyssal\_creatures\_sprites/Creature\_4.png
creature5 → assets/abyssal\_creatures\_sprites/Creature\_5.png
```

### Decoração do fundo oceânico:

```
algae      → assets/coral\_sprites/algae2.png
concha     → assets/coral\_sprites/Concha.png
coral1     → assets/coral\_sprites/Coral1.png
coral3     → assets/coral\_sprites/Coral3.png   ← coral rosa
rock       → assets/coral\_sprites/rock.png
chest      → assets/coral\_sprites/chest.png
```
...
|`enemy\_shot`|14×22|Orbe bioluminescente multicamadas (ver §4.1)|
...
### 4.1 — Textura `enemy\_shot` (orbe abissal)
...
g.generateTexture('enemy\_shot', 14, 22);
```
...
1. Gradiente vertical `0x0a1a4a` → `0x000510` em 20 faixas
2. 3 feixes de luz triangulares `0x5599ff` alpha 0.05
3. Areia: `fillRect(0,548,W,52)` cor `0xb89560` + elipses irregulares `0xd4b07a`
4. Partículas `bgBubble` subindo do fundo (emitter com `blendMode: 'ADD'`)
...
* Cria projétil `enemy\_shot` com `velocityY = 250 + wave\*20`
...
1. `createWaterGradient()`
2. `createSandFloor()`
3. `createParallaxBackground()`
4. `createBubbleSystem()`
5. `createBackgroundFish()`
6. `createDecorativeCorals()`
...
Assets não usam `seaweed\_img`. Todos têm `setOrigin(0.5, 1)` + ângulo aleatório leve (±3°).
...
1. **Tudo em um único `.html`** — sem arquivos externos além do CDN do Phaser e Google Fonts.
2. **Inicializar `enemyFireTimer` ANTES de `spawnWave()`** para evitar erro de referência nula.
3. **Inicializar todos os grupos de física ANTES** de criar qualquer sprite que dependa deles.
4. **Scanlines CSS** devem usar `height: 548px` (não `inset: 0`), para não cobrir a areia.
5. **Profundidade do jogador:** depth=5. Inimigos: depth=3. Projéteis: depth=4. HUD: depth 20–22.
6. O **parallax** é redesenhado a cada frame (chamada em `update()`); não usar sprites estáticos.
7. A **velocidade dos inimigos** é recalculada após cada morte com o multiplicador de escassez.
8. `fadeIn(400)` na câmera ao iniciar `GameScene.create()`.
9. O `chest` (baú do tesouro) fica posicionado na zona central como elemento focal — não balança.
